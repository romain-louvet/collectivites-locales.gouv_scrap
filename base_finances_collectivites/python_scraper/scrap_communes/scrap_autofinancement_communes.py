...
# requete javascript
# PARAM = 4 = autofinancement
part1 = "openWithPostData('tableau.php',{'ICOM':'"
part2 = "','DEP':'"
part3 = "','TYPE':'BPS','PARAM':'4','EXERCICE':'"
part4 = "'})"
...
for exercice in exercices:  
    ...
    if os.path.isfile(sortie):
        ...
        fichier.write("INSEE_COM;FNCTNMT_MILLEUROS;FNCTNMT_EUROHAB;FNCTNMT_MOYSTRATE;")
        fichier.write("CAF_MILLEUROS;CAF_EUROHAB;CAF_MOYSTRATE;")
        fichier.write("NETTE_MILLEUROS;NETTE_EUROHAB;NETTE_MOYSTRATE\n")
        ...
        for code in codes:
            ...
            if "Données non disponibles" in html_source:
                fichier.write(code+";NA;NA;NA;NA;NA;NA;NA;NA;NA\n")
                ...
                trouver_chiffres = re.findall('<td class="montantpetita">(.*?)\xa0</td>\n',html_source)

                # gestion erreur: si ne trouve pas toutes les valeurs
                # recherchees = erreur, sinon recupere les valeurs
                if len(trouver_chiffres)!=9:
                    fichier.write(code+";NA;NA;NA;NA;NA;NA;NA;NA;NA\n")

                else:                  
                    fonctionnement = trouver_chiffres[0:3]
                    caf = trouver_chiffres[3:6]
                    nette = trouver_chiffres[6:9]
                    
                    fichier.write(code+";"+str(fonctionnement[0])+";"+str(fonctionnement[1])+";"+str(fonctionnement[2])+";")
                    fichier.write(str(caf[0])+";"+str(caf[1])+";"+str(caf[2])+";")
                    fichier.write(str(nette[0])+";"+str(nette[1])+";"+str(nette[2])+"\n")
